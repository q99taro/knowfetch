...
"""
    ...
    def summarize_article(self, article_text: str) -> str:
        ...
        """
        prompt = f"""
你是一個資深的 AI 軟體架構師與技術實戰專家。你的任務是從下方的技術文本中，提煉出對「資深 AI 工程師」具備高度實作價值與複習意義的技術重點，並撰寫成一篇排版精美、結構清晰的 Markdown 筆記。
...
當提取這類知識時，必須將「作者的解釋（為什麼這樣寫更好、更安全、更快？解決了什麼痛點？）」與「具體的程式碼」完整保留，保留充足的前後文脈絡。

【嚴格排除黑名單 (絕對不要擷取)】：
  1. 純理論、底層架構抽象與概念性框架介紹：只關注對實作寫 Code 有直接幫助的細節。
  2. Python 內建庫與基礎 I/O 操作：排除對資深工程師毫無價值的目錄建立、讀寫等基本操作。
  3. 初階通用基礎語法：絕對排除 for-loop、if-else、基本資料型態操作。
  4. 毫無技術細節的通識名詞定義。

【排版規範】：
請將提煉出的精華透過適當的 Markdown 標題 (##, ###)、重點清單 (-) 進行排版。
每個知識點請包含：
1. **情境與痛點**：這段程式碼或技巧解決了什麼問題？
2. **核心觀念 / 最佳實踐**：為什麼這是一個好方法？
3. **程式碼範例**：若原文有程式碼，請務必使用 Markdown 語法 (```python) 包含進來，並保留原始縮排。

若整篇文章完全沒有符合要求的進階技術重點，請直接回傳：「本篇文章無進階實作知識點」。否則，請直接輸出你的 Markdown 筆記（請使用繁體中文）。
